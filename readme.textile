...
If the string is an encoded type and the bottom two bits of the length are either (00,01,or 10), the string is loaded as an integer, as described above in the integer-loading section, and converted to a string.
# If the bottom two bits of the length are set to 00, the string is a 8-bit

If the string is encoded and the remaining 6 bits of the length, it is loaded as an lzf compressed string, as described below.
